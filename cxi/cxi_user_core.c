...
/* Expected size for incoming requests. */
static const struct cmd_info cmds_info[CXI_OP_MAX] = {
	[CXI_OP_LNI_ALLOC] = { sizeof(struct cxi_lni_alloc_cmd), "LNI_ALLOC",
			       cxi_user_lni_alloc, },
	[CXI_OP_LNI_FREE] = { sizeof(struct cxi_lni_free_cmd), "LNI_FREE",
			      cxi_user_lni_free, },
	[CXI_OP_DOMAIN_RESERVE] = { sizeof(struct cxi_domain_reserve_cmd),
				    "DOMAIN_RESERVE",
				    cxi_user_domain_reserve, },
	[CXI_OP_DOMAIN_ALLOC] = { sizeof(struct cxi_domain_alloc_cmd),
				  "DOMAIN_ALLOC",
				  cxi_user_domain_alloc, },
	[CXI_OP_DOMAIN_FREE] = { sizeof(struct cxi_domain_free_cmd),
				 "DOMAIN_FREE",
				 cxi_user_domain_free, },
	[CXI_OP_CP_ALLOC] = { sizeof(struct cxi_cp_alloc_cmd), "CP_ALLOC",
			      cxi_user_cp_alloc, },
	[CXI_OP_CP_FREE] = { sizeof(struct cxi_cp_free_cmd), "CP_FREE",
			     cxi_user_cp_free, },
	[CXI_OP_CQ_ALLOC] = { sizeof(struct cxi_cq_alloc_cmd), "CQ_ALLOC",
			      cxi_user_cq_alloc, },
	[CXI_OP_CQ_FREE] = { sizeof(struct cxi_cq_free_cmd), "CQ_FREE",
			     cxi_user_cq_free, },
	[CXI_OP_CQ_ACK_COUNTER] = { sizeof(struct cxi_cq_ack_counter_cmd),
				    "CQ_ACK_COUNTER",
				    cxi_user_cq_ack_counter },
	[CXI_OP_ATU_MAP] = { sizeof(struct cxi_atu_map_cmd), "ATU_MAP",
			     cxi_user_atu_map, },
	[CXI_OP_ATU_UNMAP] = { sizeof(struct cxi_atu_unmap_cmd), "ATU_UNMAP",
			       cxi_user_atu_unmap, },
	[CXI_OP_ATU_UPDATE_MD] = { sizeof(struct cxi_atu_update_md_cmd),
				     "ATU_UPDATE_MD",
				     cxi_user_update_md, },
	[CXI_OP_EQ_ALLOC] = { sizeof(struct cxi_eq_alloc_cmd), "EQ_ALLOC",
			      cxi_user_eq_alloc, },
	[CXI_OP_EQ_FREE] = { sizeof(struct cxi_eq_free_cmd), "EQ_FREE",
			     cxi_user_eq_free, },
	[CXI_OP_EQ_RESIZE] = { sizeof(struct cxi_eq_resize_cmd), "EQ_RESIZE",
			       cxi_user_eq_resize, },
	[CXI_OP_EQ_RESIZE_COMPLETE] = {
				sizeof(struct cxi_eq_resize_complete_cmd),
				"EQ_RESIZE_COMPLETE",
				cxi_user_eq_resize_complete, },
	[CXI_OP_PTE_ALLOC] = { sizeof(struct cxi_pte_alloc_cmd), "PTE_ALLOC",
			       cxi_user_pte_alloc, },
	[CXI_OP_PTE_FREE] = { sizeof(struct cxi_pte_free_cmd), "PTE_FREE",
			      cxi_user_pte_free, },
	[CXI_OP_PTE_MAP] = { sizeof(struct cxi_pte_map_cmd), "PTE_MAP",
			     cxi_user_pte_map, },
	[CXI_OP_PTE_UNMAP] = { sizeof(struct cxi_pte_unmap_cmd), "PTE_UNMAP",
			       cxi_user_pte_unmap, },
	[CXI_OP_PTE_LE_INVALIDATE] = {
				sizeof(struct cxi_pte_le_invalidate_cmd),
				"PTE_LE_INVALIDATE",
				cxi_user_pte_le_invalidate, },
	[CXI_OP_PTE_STATUS] = { sizeof(struct cxi_pte_status_cmd), "PTE_STATUS",
				cxi_user_pte_status, },
	[CXI_OP_PTE_TRANSITION_SM] = { sizeof(struct cxi_pte_transition_sm_cmd),
				"PTE_TRANSITION_SM",
				cxi_user_pte_transition_sm, },
	[CXI_OP_WAIT_ALLOC] = { sizeof(struct cxi_wait_alloc_cmd), "WAIT_ALLOC",
				cxi_user_wait_alloc, },
	[CXI_OP_WAIT_FREE] = { sizeof(struct cxi_wait_free_cmd), "WAIT_FREE",
			       cxi_user_wait_free, },
	[CXI_OP_CT_ALLOC] = { sizeof(struct cxi_ct_alloc_cmd), "CT_ALLOC",
			      cxi_user_ct_alloc, },
	[CXI_OP_CT_WB_UPDATE] = { sizeof(struct cxi_ct_wb_update_cmd),
				  "CT_WB_UPDATE", cxi_user_ct_wb_update, },
	[CXI_OP_CT_FREE] = { sizeof(struct cxi_ct_free_cmd), "CT_FREE",
			     cxi_user_ct_free, },
	[CXI_OP_MAP_CSRS] = { sizeof(struct cxi_map_csrs_cmd), "MAP_CSRS",
			      cxi_user_map_csrs, true, },
	[CXI_OP_DEV_ALLOC_RGROUP] = { sizeof(struct cxi_dev_alloc_rgroup_cmd),
				      "DEV_ALLOC_RGROUP",
				      cxi_user_dev_alloc_rgroup, true, },
	[CXI_OP_DEV_GET_RGROUP_IDS] = { sizeof(struct cxi_dev_get_rgroup_ids_cmd),
					"DEV_GET_RGROUP_IDS",
					cxi_user_dev_get_rgroup_ids, true, },
	[CXI_OP_RGROUP_RELEASE] = { sizeof(struct cxi_rgroup_release_cmd),
					"RGROUP_RELEASE",
					cxi_user_rgroup_release, true, },
	[CXI_OP_RGROUP_ENABLE] = { sizeof(struct cxi_rgroup_enable_cmd),
					"RGROUP_ENABLE",
					cxi_user_rgroup_enable, true, },
	[CXI_OP_RGROUP_DISABLE] = { sizeof(struct cxi_rgroup_disable_cmd),
					"RGROUP_DISABLE",
					cxi_user_rgroup_disable, true, },
	[CXI_OP_RGROUP_GET_INFO] = { sizeof(struct cxi_rgroup_get_info_cmd),
					"RGROUP_GET_INFO",
					cxi_user_rgroup_get_info, true, },
	[CXI_OP_RGROUP_ADD_RESOURCE] = { sizeof(struct cxi_rgroup_add_resource_cmd),
					"RGROUP_ADD_RESOURSE",
					cxi_user_rgroup_add_resource, true, },
	[CXI_OP_RGROUP_DELETE_RESOURCE] = { sizeof(struct cxi_rgroup_delete_resource_cmd),
					"RGROUP_DELETE_RESOURSE",
					cxi_user_rgroup_delete_resource, true, },
	[CXI_OP_RGROUP_GET_RESOURCE] = { sizeof(struct cxi_rgroup_get_resource_cmd),
					"RGROUP_GET_RESOURSE",
					cxi_user_rgroup_get_resource, true, },
	[CXI_OP_RGROUP_GET_RESOURCE_TYPES] = { sizeof(struct cxi_rgroup_get_resource_types_cmd),
					"RGROUP_GET_RESOURSE_TYPES",
					cxi_user_rgroup_get_resource_types, true, },
	[CXI_OP_RGROUP_ADD_AC_ENTRY] = { sizeof(struct cxi_rgroup_add_ac_entry_cmd),
					"RGROUP_ADD_AC_ENTRY",
					cxi_user_rgroup_add_ac_entry, true, },
	[CXI_OP_RGROUP_DELETE_AC_ENTRY] = { sizeof(struct cxi_rgroup_delete_ac_entry_cmd),
					"RGROUP_DELETE_AC_ENTRY",
					cxi_user_rgroup_delete_ac_entry, true, },
	[CXI_OP_RGROUP_GET_AC_ENTRY_IDS] = { sizeof(struct cxi_rgroup_get_ac_entry_ids_cmd),
					"RGROUP_GET_AC_ENTRY_IDS",
					cxi_user_rgroup_get_ac_entry_ids, true, },
	[CXI_OP_RGROUP_GET_AC_ENTRY_DATA_BY_ID] = {
					sizeof(struct cxi_rgroup_get_ac_entry_data_by_id_cmd),
					"RGROUP_GET_AC_ENTRY_DATA_BY_ID",
					cxi_user_rgroup_get_ac_entry_data_by_id, true, },
	[CXI_OP_RGROUP_GET_AC_ENTRY_ID_BY_DATA] = {
					sizeof(struct cxi_rgroup_get_ac_entry_id_by_data_cmd),
					"RGROUP_GET_AC_ENTRY_ID_BY_DATA",
					cxi_user_rgroup_get_ac_entry_id_by_data, true, },
	[CXI_OP_RGROUP_GET_AC_ENTRY_ID_BY_USER] = {
					sizeof(struct cxi_rgroup_get_ac_entry_id_by_user_cmd),
					"RGROUP_GET_AC_ENTRY_ID_BY_USER",
					cxi_user_rgroup_get_ac_entry_id_by_user, true, },
	[CXI_OP_SVC_GET] = { sizeof(struct cxi_svc_get_cmd), "SVC_GET",
			       cxi_user_svc_get, },
	[CXI_OP_SVC_LIST_GET] = { sizeof(struct cxi_svc_list_get_cmd), "SVC_LIST_GET",
			       cxi_user_svc_list_get, },
	[CXI_OP_SVC_RSRC_LIST_GET] = { sizeof(struct cxi_svc_rsrc_list_get_cmd), "SVC_RSRC_LIST_GET",
			       cxi_user_svc_rsrc_list_get, },
	[CXI_OP_SVC_RSRC_GET] = { sizeof(struct cxi_svc_rsrc_get_cmd), "SVC_RSRC_GET",
			       cxi_user_svc_rsrc_get, },
	[CXI_OP_SVC_ALLOC] = { sizeof(struct cxi_svc_alloc_cmd), "SVC_ALLOC",
			       cxi_user_svc_alloc, true, },
	[CXI_OP_SVC_DESTROY] = { sizeof(struct cxi_svc_destroy_cmd),
				 "SVC_DESTROY",
				 cxi_user_svc_destroy, true, },
	...
	[CXI_OP_SBUS_OP] = { sizeof(struct cxi_sbus_op_cmd), "SBUS_OP",
			     cxi_user_sbus_op, true, },
	[CXI_OP_SERDES_OP] = { sizeof(struct cxi_serdes_op_cmd), "SERDES_OP",
			       cxi_user_serdes_op, true, },
	[CXI_OP_GET_DEV_PROPERTIES] = {
		sizeof(struct cxi_get_dev_properties_cmd),
		"GET_DEV_PROPERTIES",
		cxi_get_dev_properties, },
	[CXI_OP_EQ_ADJUST_RESERVED_FC] = {
		sizeof(struct cxi_eq_adjust_reserved_fc_cmd),
		"EQ_ADJUST_RESERVED_FC",
		cxi_user_eq_adjust_reserved_fc,	},
	[CXI_OP_INBOUND_WAIT] = {
		sizeof(struct cxi_inbound_wait_cmd),
		"INBOUND_WAIT",
		cxi_user_inbound_wait, },
	[CXI_OP_DEV_INFO_GET] = { sizeof(struct cxi_dev_info_get_cmd),
		"DEV_INFO_GET",
		cxi_user_dev_info_get, },
};
